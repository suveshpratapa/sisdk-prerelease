...
Shows the Wake-up link state, among `disabled`, `enabled` (only for WED), `linking` and `linked`.

`OPENTHREAD_CONFIG_WAKEUP_COORDINATOR_ENABLE` or `OPENTHREAD_CONFIG_WAKEUP_END_DEVICE_ENABLE` are required.
